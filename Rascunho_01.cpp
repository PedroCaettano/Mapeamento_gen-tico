...
void inicializarPopulacao(vector<Individuo>& populacao, int tamanhoPopulacao, int numGenes) {
        ...
        for (int j = 0; j < numGenes; ++j) {
            individuo.genes.push_back(rand() % 200 - 100); // Genes estabelecidos em uma variação entre -100 e 100
        }
...
// Exibindo o melhor individuo
Individuo melhorIndividuo = populacao[0];
          for(const auto&Individuo : populacao) {
          if (Individuo.fitness < melhorIndividuo.firness) {
              melhorIndividuo = individuo;
          }
          }


// Exibindo os genes com o melhor indivíduo
